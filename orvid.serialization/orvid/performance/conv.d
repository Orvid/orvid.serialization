...
{
	alias NativeInteger = ptrdiff_t;
	alias NativeUInteger = size_t;
	...
	static if (T.sizeof < NativeInteger.sizeof)
	{
		...
		if (result != v)
			throw new ConvException("Failed to parse the string!");
		return result;
	}
	else
	{
			...
			switch (s[0])
			{
				case '0': .. case '9':
					if (v >= (T.max / 10) && (v != (T.max / 10) || s[0] - sign > maxLastDigit))
						throw new ConvException("The number overflowed!");
					v = cast(T)(v * 10 + (s[0] - '0'));
					atStart = false;
					break;
				...
				static if (T.min < 0)
				{
					case '+':
						if (!atStart)
							throw new ConvException("Invalid character!");
						break;
					case '-':
						if (atStart)
							sign = true;
